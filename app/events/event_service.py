...
class EventService:
    ...
    def load_events(self):
        try: 
            ...
            logger.warning(f"Events file not found: {config.EVENTS_FILE}. Starting with empty events.")
        except Exception as e:
    ...
    def find_events(self, 
        ...
        for event in self.__events_by_tenant.get(tenant_id, {}).values():
            if ((start_dt is None or event.timestamp >= start_dt) and
                (end_dt is None or event.timestamp <= end_dt) and
                (action is None or event.action == action) and
                (package is None or event.package == package)):
    ...
    def cleanup_old_events(self):
        ...
        for tenant_id, events_dict in self.__events_by_tenant.items():
            old_event_ids = [event_id for event_id, event in events_dict.items() if event.timestamp < cutoff_dt]
