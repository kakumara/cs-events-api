...
@pytest.mark.parametrize(
    "tenant_id, expected_count, expected_event_ids",
    [
        ("tenant1", 2, {"evt2", "evt3"}),
        ("tenant2", 0, set()),
    ],
)
def test_find_events_with_filters_start_dt(
    tmp_path, monkeypatch, tenant_id, expected_count, expected_event_ids
):
    service = prepare_service_with_data(tmp_path, monkeypatch)
...
def test_find_events_with_filters_end_dt(
    tmp_path, monkeypatch, tenant_id, expected_count, expected_event_ids
):
    service = prepare_service_with_data(tmp_path, monkeypatch)
...
@pytest.mark.parametrize(
    "tenant_id, action, expected_count, expected_event_ids",
    [
        ("tenant1", "install", 2, {"evt1", "evt3"}),
        ("tenant2", "install", 1, {"evt4"}),
    ],
)
def test_find_events_with_filters_action(
    tmp_path, monkeypatch, tenant_id, action, expected_count, expected_event_ids
):
    service = prepare_service_with_data(tmp_path, monkeypatch)
...
@pytest.mark.parametrize(
    "tenant_id, package, expected_event_ids",
    [
...
def test_find_events_with_filters_package(
    tmp_path, monkeypatch, tenant_id, package, expected_event_ids
):
    service = prepare_service_with_data(tmp_path, monkeypatch)
...
@pytest.mark.parametrize(
    "tenant_id, start_dt, end_dt, action, package, expected_event_ids",
    [
        (
            "tenant1",
        ...
        (
            "tenant1",
            datetime(2024, 1, 1, 0, 0, tzinfo=timezone.utc),
            datetime(2024, 3, 1, 0, tzinfo=timezone.utc),
            "update",
            "pkgB",
            {"evt2"},
        ),
...
def test_find_events_with_filters_combined(
    tmp_path,
    monkeypatch,
    tenant_id,
    start_dt,
